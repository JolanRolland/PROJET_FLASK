...
@app.route('/auteurs/')
def getAuteurs():
    les_auteurs = Auteur.query.all()
    return render_template('auteurs_list.html', title="R3.01 Dev Web avec Flask", auteurs=les_auteurs)

@app.route('/livres/')
def getLivres():
    les_livres = Livre.query.all()
    return render_template('livres_list.html', title="R3.01 Dev Web avec Flask - Livres", livres=les_livres
    )
...
def saveLivre():
    ...
    if unForm.validate_on_submit():
        ...
        if updatedLivre:
            ...
            updatedLivre.auteur_id = unForm.AuteurId.data
            db.session.commit()
